...
#define NUMBER_PIEXELS 10 // the number of pixels in your LED strip
...
//  float accel_vector_length = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
  ...
  int x_index = map((int) angle_x, -90, 90, 0, 9);
  int y_index = map((int) angle_y, -90, 90, 0, 9);
  
  for (int i=0; i<strip.numPixels(); i++) {
    if (i < x_index and i < y_index) strip.setPixelColor(i, 0, 255, 255); // blue
    if (i < x_index and i > y_index) strip.setPixelColor(i, 0, 0, 255); // blue
    if (i > x_index and i < y_index) strip.setPixelColor(i, 0, 255, 0); // green
    if (i > x_index and i > y_index) strip.setPixelColor(i, 0, 0, 0); // off
  };
