...
{      
  ...
  for (int i=0; i < strip.numPixels(); i++) {
    strip.setPixelColor(i, 25, 0, 25); // violet
  };
  ...
  for (int i=0; i < strip.numPixels(); i++) {
    strip.setPixelColor(i, 0, 0, 0); // violet
  };
  ...
  float prior_x = 0;
  float prior_y = 0;
  // How many degrees for a change to be detected.  Depends on noise.
  float delta_threshold = 10.0;
  int x_index = 0; // 0 to 11
  int y_index = 0; // 0 to 11
...
//  float accel_vector_length = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
  ...
  x_changed = round(angle_x / delta_threshold) - round(prior_x / delta_threshold);
  x_index = map(x_changed, -90, 90, 0, NUMBER_PIXELS);
  y_changed = round(angle_y / delta_threshold) - round(prior_y / delta_threshold);
  y_index = map(x_changed, -90, 90, 0, NUMBER_PIXELS);
  
  if (abs(x_changed) > 0)  {
      strip.setPixelColor(x_index, 25, 0, 0); // light red 
      strip.show();
      delay(1000);
      strip.setPixelColor(x_index, 0, 0, 0); // off 
      strip.show();
  }
  
    if (abs(y_changed) > 0)  {
      strip.setPixelColor(y_index, 0, 25, 0); // light green 
      strip.show();
      delay(1000);
      strip.setPixelColor(y_index, 0, 0, 0); // off 
      strip.show();
  }
 
 // not change in X or Y tilt
 if (not(x_changed and y_changed)) {
      strip.setPixelColor(5, 10, 10, 10); // light white
      strip.show();
      delay(100);
      strip.setPixelColor(5, 0, 0, 0); // off
      strip.show();
  ...
  if ((abs(x_changed) > 0) or (abs(y_changed) > 0)) {
    Serial.print("x=");
    // these two keep the column alignment the same
    if (abs(accel_angle_x) < 10) Serial.print(' ');
    if (accel_angle_x > 0) Serial.print(' ');
    Serial.print(accel_angle_x, 2); // print with two decimal digits
    Serial.print(", ");
    
    Serial.print("y=");
    // these two keep the column alignment the same
    if (abs(accel_angle_y) < 10) Serial.print(' ');
    if (accel_angle_y > 0) Serial.print(' ');
    Serial.print(accel_angle_y, 2); // print with two decimal digits
    Serial.print(", ");
    
    Serial.print("prior x=");
    // these two keep the column alignment the same
    if (abs(prior_x) < 10) Serial.print(' ');
    if (prior_x >= 0) Serial.print(' ');
    Serial.print(prior_x, 2); // print with two decimal digits
    Serial.print(", ");
    
    Serial.print("prior y=");
    // these two keep the column alignment the same
    if (abs(prior_y) < 10) Serial.print(' ');
    if (prior_y >= 0) Serial.print(' ');
    Serial.print(prior_y, 2); // print with two decimal digits
    Serial.print(", ");
    
    Serial.print("dx=");
    Serial.print(x_changed);
    Serial.print(", ");
    
    Serial.print("dy=");
    Serial.print(y_changed);
    Serial.println("");
  };
  ...
  delay(5);
    ...
    // now make the prior the current
  prior_x = angle_x;
  prior_y = angle_y;
}
