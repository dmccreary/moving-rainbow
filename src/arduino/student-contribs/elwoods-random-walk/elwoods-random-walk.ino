...
int modeCount = 4; // The number of display modes
...
void loop() {
  ...
  if (newMode == 1) {
   ...
   // select the mode
      switch (mode) {
        case 0: stayConst();break;
        case 1: systematicWalk(25);break;
        case 2: stayConst();break;
        case 3: randWalk(25);break; // Red
     }
...
void randWalk(uint8_t wait){
  ...
  while(newMode == 0){
    ...
    switch(shiftColor){
      case 0: r += (random(2) * 2 - 1) * 32; break;
      case 1: g += (random(2) * 2 - 1) * 32; break;
      case 2: b += (random(2) * 2 - 1) * 32; break;
    }
    ...
    if(i % (strip.numPixels() - 1) == 0)
      inc = -1 * inc;
  }
  
}

void systematicWalk(uint16_t wait){
  ...
  while(newMode == 0){
    ...
    if(r < 0 || r > 255 || g < 0 || g > 255 || b < 0 || b > 255) shiftColor = (shiftColor + 1) % 8;
