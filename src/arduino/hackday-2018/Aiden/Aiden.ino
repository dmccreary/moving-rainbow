...
/*        case 3: dot(0, 0, 255);break; // blue
        ...
        case 10: colorWipe(255, 0, 0);break;
...
//      for(uint16_t i=; i<=index; i++) {
//      strip.setPixelColor(i, red, blue, green);
            strip.setPixelColor(60-i, red, blue, green);
    }
  strip.show();
}
void colorWipe2(int red, int blue, int green) {
  int index = mainLoopCount % strip.numPixels();
  if (index == 0) {
    clear();
  }
...
void KITT(int red, int blue, int green) {
  int index = mainLoopCount % strip.numPixels();
  if (index == 0) {
    clear();
  }
