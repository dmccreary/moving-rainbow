...
#define NEXT_MODE_PIN 2
#define PREV_MODE_PIN 3
...
int mainLoopDelay = 20; // The delay at the end of the main loop - make it a little faster for longer strips

void setup()
{
  pinMode(LED_STRIP_PIN, OUTPUT);     // Pin 13 is output to which an LED is connected
  pinMode(NEXT_MODE_PIN, INPUT_PULLUP);     // Pin 2 is input to which a switch is connected = INT0
  pinMode(PREV_MODE_PIN, INPUT_PULLUP);    // Pin 3 is input to which a switch is connected = INT1
...
// catch the interrupts
  ...
  if (newModeDec == 1) {
    if ((millis() - lastDebounceTime) > debounceDelay) {
      Serial.print("Decrementing Mode=");
      Serial.println(mode, DEC);
      mode--;
